...
У Claude Code: `/context` — секція Memory files показує `CLAUDE.md → @AGENTS.md` і правила; `/doctor` — чи не пропонує він щось вирізати
(порожній звіт = файл не роздутий). **Сказати:** дослідження ETH (Gloaguen et al.,
...
Спершу — питання з пам'яті (те, чого немає ні в AGENTS.md, ні в правилах):

> Не відкриваючи файлів: як у Next 16.3 задати власний профіль cacheLife для 'use cache' — ключ у next.config.ts і сигнатура виклику? Відповідай з пам'яті і познач, у чому не впевнений.

Перезапустити `claude` → підтвердити проєктні сервери → `/mcp` → `/context all` (скільки контексту коштують інструменти; tool search
вантажить лише назви). Тепер те саме питання — з доками:

> Через context7 перевір для Next 16.3: як задати власний профіль cacheLife для 'use cache'. Процитуй розділ документації і виправ свою попередню відповідь, якщо треба. Нічого не редагуй.
...
(`.agent-log/`, тести). Відкрити `docs/autonomy-log.md` — так виглядає рішення про рівень довіри, записане в момент, коли його ухвалювали. Capstone здається окремо: форк репозиторію capstone + PR із заповненим шаблоном.
